		...
		if (prevData.value.length !== 0) {
			...
			timeout = setTimeout(
				() => {
					...
					if (dataQueue.value.length !== 0 && active) {
	...
	const onLongPressGestureEvent = useAnimatedGestureHandler({
		onActive: (event) => {
			...
			if (
				...
				positionY.value =
					(prevLastY + progress * (lastY - prevLastY)) * layoutSize.value.height;
			} else if (idx === 0) {
	...
	const path = useDerivedValue(() => {
		...
		if (progress.value !== 1) {
			const numOfPoints = Math.round(
				fromValue.length +
					(toValue.length - fromValue.length) * Math.min(progress.value, 0.5) * 2
			);
